...
int main(int argc, char* argv[])
{
	...
	string filename ="d:/sample/test1/aa-1.jpg";
	double canny_threshold = 200;
	double circle_threshold = 1;
	int iterations = 500;
...
void circleRANSAC(Mat& image, std::vector<Vec3f>& circles, double canny_threshold, double circle_threshold, int numIterations)
{
	...
	for (int iteration = 0; iteration < numIterations; iteration++)
	{
		...
		if (false)
		{
			...
			cvtColor(debug_image, debug_image, cv::COLOR_GRAY2RGB);
		...
		if ((float)votes.size() / (2.0 * CV_PI * radius) >= circle_threshold)
		{
			...
			if (false)
			{
				...
				cvtColor(debug_image2, debug_image2, COLOR_GRAY2RGB);
