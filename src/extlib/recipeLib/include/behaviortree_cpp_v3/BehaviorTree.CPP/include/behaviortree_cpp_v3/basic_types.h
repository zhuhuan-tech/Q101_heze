...
public:
  ...
  void setOptions(StringView options);
                                                  ...
                                                  StringView description)
{
  ...
  std::string str = BT::toStr(default_value);
  if (!str.empty() && str.front() == '[' && str.back() == ']')
  {
    std::string options = str.substr(1, str.size() - 2);
    std::size_t pos = options.find(',');
    std::string first = options.substr(0, pos);
    out.second.setDefaultValue(BT::toStr(first));
    out.second.setOptions(options);
  }
  else
  {
