...
int main ()
{
    ...
    try
    {
        ...
        // do absolute motions
        if (!Hydra_MOV (iD, sAxis.c_str (), &targetPos))
        {
        ...
        // do relative motions
        if (!Hydra_MVR (iD, sAxis.c_str (), &targetPos))
        {
...
double GetMinPositionLimit (int ID, std::string axis)
{
    ...
    if (!Hydra_qTMN (ID, axis.c_str (), &minPosLimit))
    {
...
double GetMaxPositionLimit (int ID, std::string axis)
{
    ...
    if (!Hydra_qTMX (ID, axis.c_str (), &maxPosLimit))
    {
