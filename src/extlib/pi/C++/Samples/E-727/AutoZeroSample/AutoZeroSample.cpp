...
int main ()
{
    ...
    if (iD < 0)
    {
        std::cout << "Unable to connect to " << CONTROLLER_TYPE_C727 << ".";
        return 1;
    }
    ...
    try
    {
        ...
        if (!AutoZeroIfNeeded (iD, sAxis))
        {
            throw new std::runtime_error ("AutoZero failed.");
        }
...
int OpenConnection (ConnectionType type, std::string hostName)
{
    ...
    {
        case Dialog:
            iD = PI_InterfaceSetupDlg (CONTROLLER_TYPE_C727.c_str ());
            connType = "Dialog";
            break;
        case RS232:
        ...
        case USB:
            ...
            int noDevices = PI_EnumerateUSB (connectedUsbController, 1024, CONTROLLER_TYPE_C727.c_str ());
