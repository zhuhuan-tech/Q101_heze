...
{
	Mat img, gray;
	img = imread("D:\\sample\\test1\\mdpa-1.tif");
	...
	for (size_t i = 0; i < circles.size(); i++)
	{
		...
		if (ratio >= 0.85)
		{
			putText(img, "Quarter", Point(center.x - radius, center.y + radius + 15), FONT_HERSHEY_COMPLEX_SMALL, 0.7, Scalar(0, 255, 255), 0.4);
			change = change + .25;
		}
		else if ((ratio >= 0.68) && (ratio < 85))
		{
			putText(img, "5-cents", Point(center.x - radius, center.y + radius + 15), FONT_HERSHEY_COMPLEX_SMALL, 0.7, Scalar(0, 255, 255), 0.4);
			change = change + .05;
		}
		else if ((ratio >= 0.60) && (ratio < .68))
		{
			putText(img, "1-cent", Point(center.x - radius, center.y + radius + 15), FONT_HERSHEY_COMPLEX_SMALL, 0.7, Scalar(0, 255, 255), 0.4);
			change = change + .01;
		}
		else if ((ratio >= 0.40) && (ratio < .60))
		{
			putText(img, "Dime", Point(center.x - radius, center.y + radius + 15), FONT_HERSHEY_COMPLEX_SMALL, 0.7, Scalar(0, 255, 255), 0.4);
			change = change + .1;
		}

	}

	putText(img, "Total Money:" + to_string(change), Point(img.cols / 10, img.rows - img.rows / 10), FONT_HERSHEY_COMPLEX_SMALL, 0.7, Scalar(0, 255, 255), 0.6);
