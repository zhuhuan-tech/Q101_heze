...
namespace SplineSpace		
{
	...
	public:
		...
		Spline(double* x0,double* y0,int& num,
			BoundaryCondition bc=GivenSecondOrder,double leftBoundary=0,double rightBoundary=0);
	...
	private:
		...
		//属性
		double* GivenX;	//已知数据的自变量
		double* GivenY;	//已知数据的因变量
		int GivenNum;		//给定的已知数据点数
		BoundaryCondition Bc;	//边界类型
		double LeftB;
		double RightB;
