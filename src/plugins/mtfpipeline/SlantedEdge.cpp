...
void SetSlantedPixel(double pl)
{
	SlantedEdge::pixel = pl;
}
double getSlantedPixel()
{
    return SlantedEdge::pixel * SlantedEdge::binning;
}

void SetBinning(int binning)
{
    SlantedEdge::binning = binning;
}

int getBinning()
{
    return SlantedEdge::binning;
}
//检查ROI区域方向以及对比度
int calculate_mtf_slantededge(Mat& roi, double** Freq, double** Mtf, double** Esf_Lsf_x, double** Esf, double** Lsf, int* bin_len, double* esf_len, double* slope)
{
	...
	//Mat roi_check;
	//roi.convertTo(roi_check, CV_8SC1);
...
double quick_esf(Mat img)
{
	...
	double esf_len = 0;////20200717 calculate_esf(esf, esf_x, len, 0.1, 0.9);
...
{
	if (roi.channels() == 3)//转灰度图
		cvtColor(roi, roi, CV_BGR2GRAY);

	double vlu, vru, vll, vrl;
	double topdiff, botdiff, lftdiff, rgtdiff;
	//vlu = roi.at<uchar>(0, 0);

	if (roi.depth() == CV_16U)
	{
		vlu = (roi.at<ushort>(0, 0) + roi.at<ushort>(0, 1) + roi.at<ushort>(1, 0) + roi.at<ushort>(1, 1)) / 4;
		vru = (roi.at<ushort>(0, roi.cols - 1) + roi.at<ushort>(0, roi.cols - 2) + roi.at<ushort>(1, roi.cols - 1) + roi.at<ushort>(1, roi.cols - 2)) / 4;
		vll = (roi.at<ushort>(roi.rows - 1, 0) + roi.at<ushort>(roi.rows - 1, 1) + roi.at<ushort>(roi.rows - 2, 0) + roi.at<ushort>(roi.rows - 2, 1)) / 4;
		vrl = (roi.at<ushort>(roi.rows - 1, roi.cols - 1) + roi.at<ushort>(roi.rows - 1, roi.cols - 2) + roi.at<ushort>(roi.rows - 2, roi.cols - 1) + roi.at<ushort>(roi.rows - 2, roi.cols - 2)) / 4;

	}
	else if(roi.depth() == CV_8U)
	{
		vlu = (roi.at<uchar>(0, 0) + roi.at<uchar>(0, 1) + roi.at<uchar>(1, 0) + roi.at<uchar>(1, 1)) / 4;
		vru = (roi.at<uchar>(0, roi.cols - 1) + roi.at<uchar>(0, roi.cols - 2) + roi.at<uchar>(1, roi.cols - 1) + roi.at<uchar>(1, roi.cols - 2)) / 4;
		vll = (roi.at<uchar>(roi.rows - 1, 0) + roi.at<uchar>(roi.rows - 1, 1) + roi.at<uchar>(roi.rows - 2, 0) + roi.at<uchar>(roi.rows - 2, 1)) / 4;
		vrl = (roi.at<uchar>(roi.rows - 1, roi.cols - 1) + roi.at<uchar>(roi.rows - 1, roi.cols - 2) + roi.at<uchar>(roi.rows - 2, roi.cols - 1) + roi.at<uchar>(roi.rows - 2, roi.cols - 2)) / 4;

	}
	else
	{
		vlu = (roi.at<float>(0, 0) + roi.at<float>(0, 1) + roi.at<float>(1, 0) + roi.at<float>(1, 1)) / 4;
		vru = (roi.at<float>(0, roi.cols - 1) + roi.at<float>(0, roi.cols - 2) + roi.at<float>(1, roi.cols - 1) + roi.at<float>(1, roi.cols - 2)) / 4;
		vll = (roi.at<float>(roi.rows - 1, 0) + roi.at<float>(roi.rows - 1, 1) + roi.at<float>(roi.rows - 2, 0) + roi.at<float>(roi.rows - 2, 1)) / 4;
		vrl = (roi.at<float>(roi.rows - 1, roi.cols - 1) + roi.at<float>(roi.rows - 1, roi.cols - 2) + roi.at<float>(roi.rows - 2, roi.cols - 1) + roi.at<float>(roi.rows - 2, roi.cols - 2)) / 4;
	}

	try {
		topdiff = (vru - vlu) / (vru + vlu);
		botdiff = (vrl - vll) / (vrl + vll);
		lftdiff = (vll - vlu) / (vll + vlu);
		rgtdiff = (vrl - vru) / (vrl + vru);

		double th_value = 0.1;
		/* Check the direction of edge and change to vertical one */
		if ((fabs(topdiff + botdiff) < fabs(lftdiff + rgtdiff)) && lftdiff * rgtdiff > 0) {
			if (fabs(lftdiff) < th_value || fabs(rgtdiff) < th_value) {
				//printf("Warning: Low edge contrast (< 20%% modulation)\n");
				return false;
			}
		...
		if ((fabs(topdiff + botdiff) > fabs(lftdiff + rgtdiff)) && topdiff * botdiff > 0) {
			if (fabs(topdiff) < th_value || fabs(botdiff) < th_value) {
				//printf("Warning: Low edge contrast (< 20%% modulation)\n");
				return false;
			}
			direction = VERTICAL;
...
bool roi_normilized(Mat &roi, double* farea, int& size_x, int& size_y, int& len)
{
	...
	Mat roi_int;
	roi.convertTo(roi_int, CV_32FC1);
	int r = roi_int.rows;
	int c = roi_int.cols;
	...
	for (int i = 0; i < roi_int.rows; i++)
	{
		for (int j = 0; j < roi_int.cols; j++)
		{
			...
			farea[dex] = double((roi_int.at<float>(i, j) - minv)/ (maxv - minv));
			dex++;
		}
	}
	...
	double *slope, int *numcycles, int *pcnt2, double *off, double *R2)
{
	...
	// 定位质心
	err = locate_centroids_new(farea, temp, shifts, size_x, size_y, &offset1,true);
	if (err != 0) { return 2; }
	...
	//}
	vector<double> k;
	polyfit(temp, shifts, size_y, 1, k);
	//offset2 = getY(size_y, k);
	*slope = k[1];
	...
	{
		(*esf_lsf_x)[i] = i*pixel / ALPHA * 1000;
	}
	...
	reduce_pixelization(AveEdge, bin_len);
	...
	memcpy(AveTmp, AveEdge, sizeof(double) * bin_len);
	...
	double min_esf = *min_element(*esf, *esf + bin_len);
	for (i = 0; i < bin_len; i++)
	{
		(*esf)[i] = ((*esf)[i]-min_esf) / (max_esf-min_esf);
	}


	//计算10%-90%ESF宽度
	//20200717 *esf_len = calculate_esf(*esf, *esf_lsf_x, bin_len, 0.1, 0.9);

	//*esf_len = calculate_esf(*esf, *esf_lsf_x, bin_len, 0.1, 0.9);

	calculate_derivative_new(bin_len, AveEdge, &centroid, diff_window);
	...
	*len = lsf_len / 2;
	...
	for (i = 0; i<(*len); i++) {
		...
		(*freq)[i] = ((double)i / (double)(lsf_len)) / pixel * ALPHA / cos(theta);// 每个点对应的频率
		(*mtf)[i] = (double)(sfrc / AveTmp[0]);// 归一化sfr
		//cout << (*freq)[i] << ":" << (*mtf)[i]<<endl;
	}
	double delta = pixel / ALPHA;
	...
	//for (i = 1; i<(*len); i++) {
	//	double correct_diff = sin(M_PI*(*freq)[i] * (pixel / ALPHA)) / (M_PI*(*freq)[i] * (pixel / ALPHA));
	//	(*mtf)[i] = (*mtf)[i] / correct_diff;// 矫正有限元微分的MTF

	//}
	
	for (i = 1; i<(*len); i++) {
		double correct_smooth = sin(((double) half_window * 2 + 1) * M_PI*(*freq)[i] * pixel / ALPHA) / sin(M_PI*(*freq)[i] * pixel / ALPHA) / ((double)half_window * 2 + 1);
		if ((*freq)[i] < ALPHA / pixel / (double)(half_window * 2 + 1)) {
...
// offset --> 中心行质心的位置
unsigned short locate_centroids_new(double *farea, double *temp,
	double *shifts,
	...
	double dt, dt1, dt2;
	double hamming;
	double slope, offset2, R2, avar, bvar;
	double mid;
	...
	fit(size_y, temp, shifts, &slope, &offset2, &R2, &avar, &bvar);
	// 求质心C
	for (j = 0; j < size_y; j++) {
		dt = 0.0;
		dt1 = 0.0;
		for (i = 0; i < size_x - 1; i++) {
			dt2 = farea[(j*(long)size_x) + (i + 1)] - farea[(j*(long)size_x) + i];// 计算每个点的导数
			mid = (double)offset2 + slope*(double)j;
			if (hammingfilter) {
				hamming = 0.54 + 0.46 * cos(M_PI * ((double)i - mid) / max(mid, (double)size_x - mid));
				dt2 = dt2 * hamming;
			}
			dt += dt2 * (double)i;
			dt1 += dt2;
...
unsigned short locate_centroids_old(double *farea, double *temp,
	double *shifts,
	unsigned short size_x, unsigned short size_y,
	double *offset) {
	unsigned long i, j;
...
unsigned short fit(unsigned long ndata, double *x, double *y, double *b,
	double *a, double *R2, double *avar=nullptr, double *bvar=nullptr)
{
	unsigned long i;
...
/*****************************************************************************/
//zzz
void calculate_derivative_new(unsigned int len,double *AveEdge,
	double *centroid, int separation)
{
	...
	dt = 0.0;
	dt1 = 0.0;
	double* AveTmp= (double*)malloc(len * sizeof(double));
	for (i = 0; i< len; i++)
		AveTmp[i] = AveEdge[i];
