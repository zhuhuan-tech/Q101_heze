...
bool check_roi(Mat& roi, unsigned char& direction)
{
	...
	else if (dst8bit.depth() == CV_32FC1)
	{
		dst8bit.setTo(0, dst8bit < 0);
		cv::Mat tmp;
		cv::normalize(dst8bit, tmp, 0, 255, NORM_MINMAX);
		cv::convertScaleAbs(tmp, dst8bit);
	}
	...
	Mat row0 = img_threshold.rowRange(0, 5) + img_threshold.rowRange(img_threshold.rows - 5, img_threshold.rows);
    Mat col0 = img_threshold.colRange(0, 5) + img_threshold.colRange(img_threshold.cols - 5, img_threshold.cols);
