...
/** 'list of n-tuple' data type
    ...
    list with the allocated memory at a given time is given by:
      ...
      ntl->max_size
 */
typedef struct __declspec(dllexport)  ntuple_list_s
{
    unsigned int size;
...
void __declspec(dllexport)  free_ntuple_list(ntuple_list in);
...
image_double __declspec(dllexport) new_image_double(unsigned int xsize, unsigned int ysize);
...
/** LSD Simple Interface
    ...
    @return a 5-tuple list of detected line segments.
 */
ntuple_list __declspec(dllexport) lsd(image_double image);
