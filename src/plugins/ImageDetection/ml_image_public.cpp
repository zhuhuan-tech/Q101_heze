...
using namespace cv;
using namespace std;
void MLImageDetection::MLimagePublic::NaiveRemoveNoise(cv::Mat& srcImage, double pArea)
{
...
void MLImageDetection::MLimagePublic::ContoursRemoveNoise(cv::Mat& srcImage, double pArea)
{
	int i, j;
	int color = 1;
	int nRows = srcImage.rows;
...
void MLImageDetection::MLimagePublic::Clear_MicroConnected_Areas(cv::Mat src, cv::Mat& dst, double min_area)
{
	// 备份复制
	dst = src.clone();
...
bool MLImageDetection::MLimagePublic::GetAllMinLoc(Mat image, Mat templ, double sim, Scalar mask,
		...
		if (matchFlag)
		{
			for (int i = 0; i < num; i++)
			{
				...
				if (max_value > sim)
				{
					...
					rectangle(img_clon, Rect(max_Loc.x, max_Loc.y, templ_Width, templ_Height), mask,
						-1);
				}
				else
					break;
...
void MLImageDetection::MLimagePublic::readROItxtFile(string filepath, vector<cv::Rect>& roiArea)
{
...
void MLImageDetection::MLimagePublic::readPointstxtFile(string filepath, vector<cv::Point2f>& points)
{
...
void MLImageDetection::MLimagePublic::writePointsFile(string fileName, vector<cv::Point2f> points)
{
	...
	if (roiVec.size() > 0)
	{
		...
		for (int i = 0; i < roiVec.size(); i++)
		{
			if (roiVec[i].width > 0 & roiVec[i].height > 0)
				datafile1 << roiVec[i].x << ' ' << roiVec[i].y << ' ' << roiVec[i].width << ' ' << roiVec[i].height << endl;
		}
		datafile1.close();
	}
}
cv::Mat MLImageDetection::MLimagePublic::readCSVToMat(const string& filename)
{
...
void MLImageDetection::MLimagePublic::writeMatTOCSV(std::string filename, cv::Mat m)
{
	std::ofstream myfile;
...
vector<int> MLImageDetection::MLimagePublic::findPeaks(cv::Mat data, double minHeight, int minWidth, double minProminence)
{
	vector<int> xVec;
	// peaks.clear(); // 清空峰值数组
	...
	for (int i = thresh; i < data.cols - thresh; ++i)
	{
			...
			(data.at<double>(0, i) > data.at<double>(0, i - 1) && data.at<double>(0, i) >= data.at<double>(0, i + 1)))
		{
			...
			double prominence =
				data.at<double>(0, i) - max(data.at<double>(0, left + 1), data.at<double>(0, right - 1));
...
int MLImageDetection::MLimagePublic::findEdgePt(cv::Mat data)
{
	...
	vector<int> xVec = findPeaks(gx, maxV * 0.8, 0, 0);
	if (xVec.size() < 1)
		return gx.total() / 2;
	return xVec[0];
}

cv::Mat MLImageDetection::MLimagePublic::convertToUint8(const cv::Mat img)
{
	if (img.depth() == CV_8U)
		return img;
	cv::Mat tmp;
...
cv::Mat MLImageDetection::MLimagePublic::convert12bitTo8bit(cv::Mat img)
{
	cv::Mat img_gray;
	if (img.channels() == 3)
...
cv::Mat MLImageDetection::MLimagePublic::convert2ColorImage(cv::Mat img)
{
	if (img.channels() == 1)
	{
		vector<cv::Mat> imgm;
...
cv::Mat MLImageDetection::MLimagePublic::convertTo3Channels(const cv::Mat binImg)
{
...
cv::Mat MLImageDetection::MLimagePublic::convertToGrayImage(const cv::Mat img)
{
	cv::Mat img8;
...
cv::Mat MLImageDetection::MLimagePublic::getRectROIImg(cv::Mat img, cv::Rect rec)
{
	cv::Mat roi;
	if (rec.area() <= 1)
		return img;
	cv::Rect rectAnd = rec & cv::Rect(0, 0, img.cols, img.rows);
	roi = img(rectAnd).clone();
	return roi;
	if (img.data != NULL)
	{
...
cv::Mat MLImageDetection::MLimagePublic::getCircleMask(cv::Size size, int radius)
{
	...
	mask.elem(arma::find(mask1)).fill(255);
...
cv::RotatedRect MLImageDetection::MLimagePublic::getMaxAreaContourRect(cv::Mat imgth, cv::Rect& rect)
{
...
cv::Mat MLImageDetection::MLimagePublic::GetROIMat(cv::Mat img, Rect rec)
{
...
double MLImageDetection::MLimagePublic::Getdistance(cv::Point p1, cv::Point p2)
{
	double d = 0;
...
double MLImageDetection::MLimagePublic::corrlateMat90(cv::Mat A0)
{
...
double MLImageDetection::MLimagePublic::corrlateMat180(cv::Mat A)
{
	double re = 0;
	cv::Mat B;
...
double MLImageDetection::MLimagePublic::getMagnitude(cv::Mat img)
{
	cv::Mat Gx, Gy;
...
vector<cv::Point> MLImageDetection::MLimagePublic::drawline(double k, double b)
{
...
vector<cv::Point> MLImageDetection::MLimagePublic::drawline(vector<double> line)
{
	vector<cv::Point> point;
	if (line.size() == 3)
	{
...
double MLImageDetection::MLimagePublic::median(vector<double> valVec)
{
...
double MLImageDetection::MLimagePublic::median(double a1, double a2, double a3)
{
	vector<double> a;
	a.push_back(a1);
	a.push_back(a2);
...
vector<double> MLImageDetection::MLimagePublic::LineInf(cv::Point2f p1, cv::Point2f p2)
{
...
cv::Point2f MLImageDetection::MLimagePublic::getintersection(double k1, double b1, double k2, double b2)
{
	cv::Point2f point;
	double x = 0, y = 0;
	x = (b2 - b1) / (k1 - k2+1e-6);
...
void MLImageDetection::MLimagePublic::writeCSV(string filename, Mat m)
{
	ofstream myfile;
	myfile.open(filename.c_str());
	myfile << cv::format(m, cv::Formatter::FMT_CSV) << std::endl;
...
cv::Point2f MLImageDetection::MLimagePublic::get2LineIntersectionPoint(cv::Point2f lineOnePt1, cv::Point2f lineOnePt2,
...
bool MLImageDetection::MLimagePublic::sort_user(const vector<double> x, vector<double>& result, vector<int>& index)
{
	bool flag = true;
	vector<double> x1(x);
...
cv::Mat MLImageDetection::MLimagePublic::linspace(double begin, double finish, int number)
{
...
double MLImageDetection::MLimagePublic::percentile(const arma::vec& data, double p)
{
	p = p / 100;
	if (p < 0 || p > 1.0) {
	...
	// 对数据进行排序
	arma::vec sorted_data = arma::sort(data);
	// 计算百分位数的位置
	double index = p * (sorted_data.n_elem - 1);
	int lower_index = static_cast<int>(std::floor(index));
	double fraction = index - lower_index;

	// 线性插值
	if (lower_index + 1 < sorted_data.n_elem) {
...
double MLImageDetection::MLimagePublic::getPercentile(cv::Mat seqMat, cv::Mat mask, double p)
{
...
arma::mat MLImageDetection::MLimagePublic::openCVMatToArmaMat(cv::Mat cv_mat)
{
	cv::Mat cv_mat1;
	cv_mat.convertTo(cv_mat1, CV_64FC1);
	arma::mat ar_mat_1_t(reinterpret_cast<double*>(cv_mat.data), cv_mat.cols, cv_mat.rows);
	arma::mat ar_mat_1 = ar_mat_1_t.t();
	return ar_mat_1;
}

cv::Mat MLImageDetection::MLimagePublic::armaMatToOpenCVMat(arma::mat ar_mat)
{
	arma::mat ar_mat_t = ar_mat.t();
	cv::Mat cv_mat(ar_mat.n_rows, ar_mat.n_cols, CV_64FC1, ar_mat_t.memptr());
	return cv_mat;
}

cv::Mat MLImageDetection::MLimagePublic::armaVecToOpenCVMat(arma::vec ar_vec)
{
	cv::Mat cv_mat_1(ar_vec.n_rows, ar_vec.n_cols, CV_64FC1, ar_vec.memptr());
	return cv_mat_1;
}

arma::vec MLImageDetection::MLimagePublic::openCVMatToArmaVec(cv::Mat cv_mat)
{
	cv_mat.convertTo(cv_mat, CV_64FC1);
	arma::mat ar_vec_1_t(reinterpret_cast<double*>(cv_mat.data), cv_mat.cols, cv_mat.rows);
	//arma::rowvec ar_vec_1 = ar_vec_1_t.t();
	arma::vec vec1 = arma::vectorise(ar_vec_1_t);
	return vec1;
}
double MLImageDetection::MLimagePublic::deg2rad(double deg)
{
...
double MLImageDetection::MLimagePublic::calculateMatMedian(cv::Mat roi)
{
	...
	cv::Mat roi64;
	roi.convertTo(roi64, CV_64FC1);
	arma::mat roimat(reinterpret_cast<double*>(roi64.data), roi64.cols, roi64.rows);
	double median = arma::median(arma::vectorise(roimat));
	return median;
}

cv::Mat MLImageDetection::MLimagePublic::getRotationImg(cv::Mat img, double angle)
{
	if (angle == 0)
		return img;
	cv::Mat rotatedImg;
...
vector<double> MLImageDetection::MLimagePublic::line_fit_orthogonal(Mat abs_grad)
{
	vector<double> point_x, point_y;
	for (int j = 0; j < abs_grad.rows; j++)
	{
...
cv::Point2f MLImageDetection::MLimagePublic::calLinesCross(float a1, float b1, float c1, float a2, float b2, float c2)
{
...
cv::Point2f MLImageDetection::MLimagePublic::calLinesIntersection(cv::Vec4f line1, cv::Vec4f line2)
{
	//TODO:: 判断是否为0
...
vector<double> MLImageDetection::MLimagePublic::unique(vector<double> xVec, int thresh)
{
	int xNum = 1;
	vector<double>xUniq;
	if (xVec.size() > 0)
...
int MLImageDetection::MLimagePublic::findIndex(vector<cv::Point2f> pts, cv::Point2f pt0, double thresh)
{
	int index = -1;
	vector<double>dis;
...
int MLImageDetection::MLimagePublic::findNeighborNum(vector<double> xVec, double x0, double thresh)
{
	int num = 0;
...
int MLImageDetection::MLimagePublic::findNeighborNum(vector<double> xVec, double x0, vector<int>& inVec, double thresh)
{
	int num = 0;
	for (int i = 0; i < xVec.size(); i++)
	{
...
vector<double> MLImageDetection::MLimagePublic::dataClusters(vector<double> data, vector<int>& vecList, double thresh)
{
...
vector<double> MLImageDetection::MLimagePublic::dataIndexMap(vector<double> data, vector<Point> P, cv::Mat& indexMap,
	double thresh)
{
...
cv::Mat MLImageDetection::MLimagePublic::generatePointsIndexMap1(vector<cv::Point2f> ptsNew, bool extendFlag, double thresh)
{
...
cv::Mat MLImageDetection::MLimagePublic::generatePointsIndexMap(vector<cv::Point2f> ptsNew, bool extendFlag, double thresh)
{
	if (ptsNew.size() < 1)
		return Mat();
	vector<double> xVec, yVec;
...
std::vector<cv::Point> MLImageDetection::MLimagePublic::convecHull(std::vector<cv::Point>& points)
{
...
double MLImageDetection::MLimagePublic::getDistancePL(Point a, Point b, Point c)
{
	//点到直线距离(利用外积平行四边形)    
	Point ab(b.x - a.x, b.y - a.y);
...
vector<int> MLImageDetection::MLimagePublic::sortIndex(vector<double> xVec)
{
	vector<int>index;
	vector<double> x;
	x = xVec;
...
cv::Point2f MLImageDetection::MLimagePublic::getPointsCenter(vector<cv::Point2f> pts)
{
	if (pts.size() < 1)
		return cv::Point2f();
	double sumx = 0;
...
int MLImageDetection::MLimagePublic::findNeartVal(vector<double> xVec, double x0)
{
	int index = -1;
	vector<double> dis;
	for (int i = 0; i < xVec.size(); i++)
...
void MLImageDetection::MLimagePublic::circleLeastFit(const std::vector<cv::Point> points, cv::Point2f& center, double& radius)
{
	radius = 0.0f;
...
void MLImageDetection::MLimagePublic::circleLeastFit(const std::vector<cv::Point2f> points, cv::Point2f& center, double& radius)
{
	radius = 0.0f;
	double sum_x = 0.0f, sum_y = 0.0f;
	double sum_x2 = 0.0f, sum_y2 = 0.0f;
...
string MLImageDetection::MLimagePublic::get_current_dir()
{
...
string MLImageDetection::MLimagePublic::createDir()
{
...
string MLImageDetection::MLimagePublic::get_exe_dir()
{
...
string MLImageDetection::MLimagePublic::currentTimetoStr()
{
	char tmp[64];
	time_t t = time(NULL);
	tm* _tm = localtime(&t);
...
cv::Point2f MLImageDetection::MLimagePublic::getOpticalCenterAfterRotation(cv::Point2f center, cv::Size s, int rota)
{
	Point2f centerR;
	if (rota == 0)
		return center;
...
cv::Point2f MLImageDetection::MLimagePublic::getOpticalCenterAfterMirror(cv::Point2f center, cv::Size s, MirrorALG m)
{
	Point2f centerM;
	cv::Point2f c0 = Point2f(s.width / 2.0, s.height / 2.0);
	cv::Point2f dis = center - c0;
...
cv::Point2f MLImageDetection::MLimagePublic::updateOpticalCenter(cv::Point2f base, cv::Size size, int rotationAngle,
	MirrorALG mirror)
{
	cv::Size s;
...
bool MLImageDetection::MLimagePublic::Is_File_Exist(const std::string& file_path)
{
	std::ifstream file(file_path.c_str());
	return file.good();
}
bool MLImageDetection::MLimagePublic::containRect(cv::Rect a, cv::Rect b)
{
...
cv::Point2f MLImageDetection::MLimagePublic::getRectCenter(cv::Rect rect)
{
	cv::Point cpt;
...
cv::Rect MLImageDetection::MLimagePublic::updateRectByRatio(cv::Rect rect, double ratio)
{
	cv::Rect rectNew;
...
void MLImageDetection::MLimagePublic::updateRectByRatio1(cv::Rect& rect, double ratio)
{
	rect.x = rect.x * ratio;
	rect.y = rect.y * ratio;
...
cv::Rect MLImageDetection::MLimagePublic::rotateRect(cv::RotatedRect rectR, cv::Point2f center, double angle)
{
	cv::Rect rect;
	cv::Point2f pt[4];
	rectR.points(pt);
...
vector<cv::Point2f> MLImageDetection::MLimagePublic::rotatePoints(vector<cv::Point2f> pts, cv::Point2f center, double angle)
{
	cv::Mat m = cv::getRotationMatrix2D(center, angle, 1.0);
	vector<cv::Point2f> ptsR;
	cv::transform(pts, ptsR, m);
...
void MLImageDetection::MLimagePublic::updateRotateImg(cv::Mat& img, double angle)
{
...
cv::Rect MLImageDetection::MLimagePublic::updateRotateRect(cv::RotatedRect rectR, cv::Point2f center)
{
	cv::Rect rect;
	if (rectR.angle < 40)
	{
...
string MLImageDetection::MLimagePublic::numToString(double value)
{
	if (value < 1e5)
	{
...
void MLImageDetection::MLimagePublic::drawPointOnImage(cv::Mat& imgdraw, cv::Point2f pt, int r)
{
	cv::circle(imgdraw, pt, r, Scalar(0, 0, 255), -1);
}

void MLImageDetection::MLimagePublic::drawPointsOnImage(cv::Mat& imgdraw, std::vector<cv::Point2f> pts, int r,
	cv::Scalar color)
{
	for (int i = 0; i < pts.size(); i++)
	{
...
void MLImageDetection::MLimagePublic::drawRectOnImage(cv::Mat& imgdraw, cv::Rect rect, int width
)
{
	cv::rectangle(imgdraw, rect, Scalar(0, 0, 255), width);
}

void MLImageDetection::MLimagePublic::drawRectsOnImage(cv::Mat& imgdraw, std::vector<cv::Rect> rectVec, int width)
{
...
void MLImageDetection::MLimagePublic::drawCircleOnImage(cv::Mat& imgdraw, cv::Point2f pt, int r)
{
	cv::circle(imgdraw, pt, r, Scalar(0, 0, 255), 1);
}

void MLImageDetection::MLimagePublic::putTextOnImage(cv::Mat& imgdraw, std::string str, cv::Point2f pt, int width)
{
	putText(imgdraw, str, pt, FONT_HERSHEY_PLAIN, width, Scalar(0, 255, 0), width);
}

void MLImageDetection::MLimagePublic::drawLineOnImage(cv::Mat& imgdraw, cv::Point2f pt1, cv::Point2f pt2, int width)
{
