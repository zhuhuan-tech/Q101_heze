...
Q_LOGGING_CATEGORY(pluginLog, "pluginsystem", QtWarningMsg)
...
void PluginManagerPrivate::checkForProblematicPlugins()
{
    ...
    if (pluginName) {
        ...
        if (spec && !spec->isRequired()) {
            ...
            const QString pluginsMenu = tr("Help > About Plugins");
...
void PluginManagerPrivate::loadPlugin(PluginSpec *spec, PluginSpec::State destState)
{
    ...
    case PluginSpec::Initialized:
        ...
        profilingReport("<initializePlugin", spec);
        connect(spec->plugin(), SIGNAL(sendMsgToManager(PluginMetaData)), q, SLOT(recMsgFromPlugin(PluginMetaData)));
        break;
