...
cv::Vec4f CrossCenter::verticalLineFit(cv::Mat img_gray)
{
    int row = img_gray.rows;
    int col = img_gray.cols;
    double minMat, maxMat;
    ...
    for (int i0 = iteration; i0 < row; i0 = i0 + iteration) // iteration thrsh2
    {
        ...
        {
            x.push_back(j);
            double temp; // = double(img_gray.at<short>(i0, j));
            if (img_gray.depth() == CV_16U)
            {
                temp = double(img_gray.at<short>(i0, j));
            }
            else
            {
                temp = double(img_gray.at<uchar>(i0, j));
            }
            y.push_back(temp);
            // i0 = i0 + 100;
            //  cout << j << ',' << temp << endl;
        }
        double maxVec = *max_element(y.begin(), y.end());
        int maxPosition = max_element(y.begin(), y.end()) - y.begin();
        int len = CrossCenterParameter::asfilterROI;
        // int len = 10;
        int left = max(i0 - len, 0);
        int right = min(i0 + len, img_gray.rows);
        int bottom = max(maxPosition - len, 0);
        int top = min(maxPosition + len, img_gray.cols);
        cv::Rect rec = cv::Rect(bottom, left, top - bottom, right - left);
        ...
        // cv::circle(m_img_draw, cv::Point(maxPosition,i0),5, cv::Scalar(0, 0, 255));
        // cv::rectangle(m_img_draw,rec, cv::Scalar(0, 0, 255), 2);
        //  cout << stdMat.at<double>(0) << endl;
        //	cout << corr << endl;
        if (maxVec > maxMat / 10 & meanRoi(0) > meanAll(0) /*& corr > 0.1*/ &
            stdMat.at<double>(0) > 2) // data need to be change as different image,thresh1
        {
            // cout << stdMat.at<double>(0) << endl;
            vector<cv::Point2f> gaussFitdata;
            fitLineData.push_back(cv::Point2f(maxPosition, i0));
            rawData.push_back(maxPosition);
            circle(m_img_draw, cv::Point2f(maxPosition, i0), 2, cv::Scalar(255, 0, 0), -1);

            // if (findGaussianFitRawData(x, y, gaussFitdata))
            //{
            //	/*for (int i = 0; i < gaussFitdata.size(); i++)
            //	{
            //		cout  << gaussFitdata[i] << endl;
            //	}
            //	*/
            //	cv::Mat A(3, 1, CV_32FC1, cv::Scalar(0));
            //	gaussiantCurveFit(gaussFitdata, 2, A);
            ...
            //	float a2 = A.at<float>(2, 0);
            //	double total = NAN;
            //	//if ((a0 != NAN | a1 != NAN | a2 != NAN)| (a0 != -NAN | a1 != -NAN | a2 != -NAN))
            //	if (abs(a1) > 1e-6)
            //	{
            //		float fParmB = -1 / (a2);
            //		float fParmC = -a1 / (2 * a2);
            ...
            //			//cout << x << ',' << y << endl;
            //		}
            //		double maxfit = *max_element(yfit.begin(), yfit.end());
            //		int listLineFit = 0;
            ...
            //			}
            //		}
            //		// fitLineData.push_back(cv::Point2f(xfit[listLineFit], yfit[listLineFit]));
            //		// circle(img_draw, cv::Point2f(xfit[listLineFit], yfit[listLineFit]), 5, cv::Scalar(0, 0, 255),
            //-1);
            //	   //  cout << i0 << endl;
            //		fitLineData.push_back(cv::Point2f(xfit[listLineFit], i0));
            //		rawData.push_back(xfit[listLineFit]);
            //		circle(m_img_draw, cv::Point2f(xfit[listLineFit], i0), 2, cv::Scalar(255, 0, 0), -1);
            //		//cout << xfit[listLineFit] << ',' << i0 << endl;
            //	}
    ...
    if (vecList.size() > 1)
    {
        ...
        {
            fitLineDataFindal.push_back(fitLineData[vecList[i]]);
            rawDataNew.push_back(rawData[vecList[i]]);
            // circle(m_img_draw, fitLineData[vecList[i]], 5, cv::Scalar(255, 0, 0), -1);
        }
    }

    double stdev = 50;
    if (rawDataNew.size() > 0)
    {
        double sum = std::accumulate(std::begin(rawDataNew), std::end(rawDataNew), 0.0);
        double mean = sum / rawDataNew.size();
        double accum = 0.0;
        std::for_each(std::begin(rawDataNew), std::end(rawDataNew),
                      [&](const double d) { accum += (d - mean) * (d - mean); });

        stdev = sqrt(accum / (rawDataNew.size() - 1));
    }
    // cout << stdev << endl;
...
cv::Vec4f CrossCenter::horizontalLineFit(cv::Mat img_gray)
{
    int row = img_gray.rows;
    int col = img_gray.cols;
    double minMat, maxMat;
    ...
    for (int j0 = iteration; j0 < col; j0 = j0 + iteration)
    {
        ...
        for (int i = 0; i < row; i++)
        {
            ...
            if (img_gray.depth() == CV_16U)
            {
                temp = double(img_gray.at<short>(i, j0));
            }
            else
            {
                temp = double(img_gray.at<uchar>(i, j0));
            }
            y.push_back(temp);
            // i0 = i0 + 100;
            //  cout << j << ',' << temp << endl;
        ...
        int len = CrossCenterParameter::asfilterROI;
        //  int len = 10;
        int left = max(j0 - len, 0);
        int right = min(j0 + len, img_gray.cols);
        int bottom = max(maxPosition - len, 0);
        int top = min(maxPosition + len, img_gray.rows);
        cv::Rect rec = cv::Rect(left, bottom, right - left, top - bottom);
        cv::Mat roi = img_gray(rec); // img_gray(cv::Range(left,right), cv::Range(bottom,top));
        cv::Scalar meanRoi = cv::mean(roi);
        double corr = corrlateMat180(roi); // corrlateMat(roi);

        //	cv::circle(m_img_draw, cv::Point(j0,maxPosition), 5, cv::Scalar(0, 255, 0));
        // cv::rectangle(m_img_draw, rec, cv::Scalar(0, 255, 0), 2);
        //	cout << "roi=" << meanRoi(0) << endl;
        cv::Mat meanMat, stdMat;
        cv::meanStdDev(roi, meanMat, stdMat);
        //	cout << (corr ) << ",";
        //	cout << (stdMat.at<double>(0)) << endl;
        ...
        {
            vector<cv::Point2f> gaussFitdata;
            fitLineData.push_back(cv::Point2f(j0, maxPosition));
            rawData.push_back(maxPosition);
            circle(m_img_draw, cv::Point2f(j0, maxPosition), 2, cv::Scalar(0, 0, 255), -1);
            ...
            //-1);
            //	   //  cout << i0 << endl;
            //		fitLineData.push_back(cv::Point2f(j0, xfit[listLineFit]));
            //		rawData.push_back(xfit[listLineFit]);
            //		circle(m_img_draw, cv::Point2f(j0, xfit[listLineFit]), 2, cv::Scalar(0, 0, 255), -1);
            //		//cout << xfit[listLineFit] << ',' << j0 << endl;
            //	}
            //}
        }
    }
    ...
    if (vecList.size() > 1)
    {
        for (int i = 0; i < vecList.size(); i++)
        {
            fitLineDataFindal.push_back(fitLineData[vecList[i]]);
            rawDataNew.push_back(rawData[vecList[i]]);
            circle(m_img_draw, fitLineData[vecList[i]], 2, cv::Scalar(0, 255, 0), -1);
        }
    }
...
cv::Point2f CrossCenter::find_centerGaussian1(cv::Mat img)
{
	if (img.data != NULL)
    {
        cv::Mat img_gray;
        ...
        // blur(img_gray, img_gray, Size(3, 3), Point(-1, -1));
        // GaussianBlur(img_gray, img_gray, Size(5, 5), 5, 5);
        // add the image prprocessing add the contrast
        ...
        {
            img_gray1 = img_gray.clone();
            m_img_draw = convertTo3Channels(img_gray);
        }
        else
        {
            img_gray1 = convertToUint8(img_gray);
        ...
        GaussianBlur(img_gray1, img_gray1, Size(5, 5), 5, 5);
        // medianBlur(img_gray1, img_gray1, 5);

        cv::Vec4f lineVertical = verticalLineFit(img_gray1);
        cv::Vec4f lineHorizontal = horizontalLineFit(img_gray1);
