...
algorithm::MultiCrossHairDetection::MultiCrossHairDetection()
{
}
...
algorithm::MultiCrossHairRe algorithm::MultiCrossHairDetection::getMuliCrossHairCenter(cv::Mat img, int binNum)
{
    MultiCrossHairRe mlchRe;
    map<string, cv::Point2f> ptMap;
    map<string, cv::Rect> rectMap;

    string infoHeader = "---Algorithm log---MultiCrossHairDetectionALG---";
    if (img.data != NULL)
    {
        cv::Mat img8 = convertToUint8(img);
        cv::Mat img_draw = convertTo3Channels(img8);
        cv::Mat gray = convertToGrayImage(img8);
        // cv::Mat templ = cv::imread("./config/templateImg/template41.tif",0);
        cv::Mat templ = cv::imread("E:\\project\\hydrus2Imgs\\newCrossImg\\templ.tif", 0);
        vector<cv::Point2f> P;
        // P = findCrossHairCenterTemplate(img, templ);
        vector<cv::Rect> rectVec;
        P = findCrossHairCenterContour(img8, rectVec);
        if (P.size() == 14)
        {
...
algorithm::MultiCrossHairRe algorithm::MultiCrossHairDetection::getMultiCrossHairCenterBySlope(cv::Mat img,
...
//MultiCrossHairRe algorithm::MultiCrossHairDetection::getMuliCrossHairCenterByDistance(cv::Mat img, int binNum)
//{
//    MultiCrossHairRe mlchRe;
//    map<string, cv::Point2f> ptMap;
//    map<string, cv::Rect> rectMap;
//
//    string infoHeader = "---Algorithm log---getMuliCrossHairCenterByDistance---";
//    if (img.data != NULL)
//    {
//        m_binNum = binNum;
//        cv::Mat img8 = convertToUint8(img);
...
MultiCrossHairRe algorithm::MultiCrossHairDetection::getMuliCrossHairCenterByDistance(cv::Mat img, int eyeLoc,
                                                                                      int binNum)
{
    MultiCrossHairRe mlchRe;
    map<string, cv::Point2f> ptMap;
    ...
    if (img.data != NULL)
    {
        ...
        if (center.x > 1e-6 && center.y > 1e-6)
        {
            ...
            for (int i = 0; i < ptsR.size(); i++)
            {
                ...
                ptMap.insert(std::make_pair(strVec[i], ptsR[i]));
                rectMap.insert(std::make_pair(strVec[i], rect0));
                mlchRe.rectVec.push_back(rect0);
                drawRectOnImage(img_draw, rect0);
                putTextOnImage(img_draw, strVec[i], rect0.tl(), 16 / binNum); 
            }
...
algorithm::CenterCrossHairRe algorithm::MultiCrossHairDetection::getCenterCrossHairCenter(cv::Mat img, int binNum)
{
    CenterCrossHairRe re;
    cv::Point2f center(0, 0);
    string infoHeader = "---Algorithm log---getCenterCrossHairCenter---";
    if (img.data != NULL)
    {
        ...
        vector<cv::Point2f> P;
        vector<cv::Rect> rectVec;
        P = findCrossHairCenterContour(gray, rectVec);
        if (P.size() == 14)
        {
...
CenterCrossHairRe algorithm::MultiCrossHairDetection::getCenterCrossHairCenterByContour(cv::Mat img, int binNum)
{
    ...
    if (img.data != NULL)
    {
       ...
       // ContoursRemoveNoise(imgth, 8000 / (binNum * binNum));
        ...
        morphologyEx(imgth, imgth, MORPH_DILATE, kernel2);
...
cv::Mat algorithm::MultiCrossHairDetection::getImgdraw()
{
    return m_img_draw;
}
vector<cv::Point2f> algorithm::MultiCrossHairDetection::findCrossHairCenterContour(cv::Mat gray,
                                                                                   vector<cv::Rect> &rectVec)
{
...
vector<cv::Point2f> algorithm::MultiCrossHairDetection::findCrossHairCenterTemplate(cv::Mat img, cv::Mat templ)
{
    vector<cv::Point2f> cenVec;
    if (img.data != NULL && templ.data != NULL)
    {
        ...
        {
            gray = img.clone();
            cv::cvtColor(img, img_draw, COLOR_GRAY2BGR);
        }
        cv::Mat img_result;
        matchTemplate(gray, templ, img_result, TM_CCOEFF_NORMED);
        cv::Mat imgre;
        threshold(img_result, imgre, 0.8, 255, THRESH_BINARY);
...
cv::Point2f algorithm::MultiCrossHairDetection::getPointsMean(vector<cv::Point2f> pts)
{
    cv::Point2f ptM(0, 0);
    if (pts.size() == 0)
        return ptM;
...
int algorithm::MultiCrossHairDetection::findClosestPoint(vector<cv::Point2f> pts, cv::Point2f p0)
{
...
cv::Point2f algorithm::MultiCrossHairDetection::getPointByDeg(cv::Point2f center, double theta, double xdeg,
                                                              double ydeg)
{
    cv::Point2f pt;
...
void algorithm::MultiCrossHairDetection::updateCenterPoints(cv::Point2f &pt1, cv::Point2f &d1, double thetaH,
