...
using namespace std;
algorithm::MLCherkerboardDetect::MLCherkerboardDetect()
{
}
...
CheckerboardRe algorithm::MLCherkerboardDetect::detectChessboardCorner(cv::Mat img, double crRatio,  int binNum)
{
    ...
    if (img.data != NULL)
    {
        ...
        if (roiimg.data)
        {
            cbdetect::find_corners(srcbinary, corners, params);
            ...
            if (corners.p.size() > 0 & chessboard.size() > 0)
            {
                ...
                {
                    cv::circle(img_draw, cornersPointNew[i], 10, cv::Scalar(255, 0, 0), -1);
                }
            }
        }
...
CheckerboardRe algorithm::MLCherkerboardDetect::detectChessboardCorner1(cv::Mat img, double crRatio, int binNum)
{
    CheckerboardRe checkerRe;
    cv::Mat indexMap;
    vector<cv::Point2f> cornersRe;
...
CheckerboardRe algorithm::MLCherkerboardDetect::detectChessboardHoughLine(cv::Mat img, double crRatio,
    int binNum)
{
    CheckerboardRe checkerRe;
    cv::Mat indexMap;
 ...
 CheckerboardRe algorithm::MLCherkerboardDetect::detectChessboardTemplate(cv::Mat img, double crRatio, int binNum)
{
     CheckerboardRe checkerRe;
    vector<cv::Point2f> corners;
    cv::Mat indexMap;
    if (img.data != NULL)
    {
        ...
        cv::Mat templ = cv::imread("./config/templateImg/chessboardtemplate1.tif", 0);
        cv::Mat templ1 = cv::imread("./config/templateImg/chessboardtemplate2.tif", 0);
        // 自动检测中心坐标
...
CheckerboardRe algorithm::MLCherkerboardDetect::detectChessboardContour(cv::Mat img, double crRatio, int binNum)
{
    CheckerboardRe checkerRe;
    vector<cv::Point2f> corners;
    cv::Mat indexMap;
    ...
    if (img.data != NULL)
    {
        ...
        for (int i = 0; i < contours.size(); i++)
        {
            ...
            // cout << area << endl;
            if (area > 2e4 / pow(binNum, 2) & area < 2e6 / pow(binNum, 2) & ratio> 0.9 & ratio < 1.2)
            {
        ...
        {
            circle(img_draw1, pts[i], 10, Scalar(0, 0, 255), -1);
        }

        int thred1 = m_pointsClusters;
        vector<cv::Point2f> ptsNew = pointsClusters(pts, 120.0 / binNum);
        for (int i = 0; i < ptsNew.size(); i++)
        {
            circle(img_draw1, ptsNew[i], 10, Scalar(255, 0, 0), -1);
        }
        // int thred2 = HydrusParameters::xyClassification;
        int thred2 = m_xyClassification;

        cv::Mat indexMap1 = generatePointsIndexMap(ptsNew, false, 120 / binNum);
        indexMap = updateIndexMapKL(indexMap1, ptsNew, corners);
        for (int i = 0; i < corners.size(); i++)
        {
            corners[i] = corners[i];
...
CheckerboardRe algorithm::MLCherkerboardDetect::detectChessboardTemplate1(cv::Mat img, double crRatio,
    int binNum)
{
    CheckerboardRe checkerRe;
    vector<cv::Point2f> corners;
    ...
    {
        m_CRAreaPercent = crRatio;
        cv::Mat gray = convertToUint8(img);
        img_draw = convertTo3Channels(gray);
        cv::Rect rect; // getChessboardRect(img_draw);
        cv::Mat roi = GetROIMat(gray, rect);
        cv::Point2f start = cv::Point2f(rect.x, rect.y);
        cv::Mat img_draw1 = convertTo3Channels(roi);
     //   string path = get_exe_dir();
       string filepath1 =  "./config/templateImg/chessboardtemplate1.tif";
       string filepath2 =  "./config/templateImg/chessboardtemplate2.tif";
        cv::Mat templ = cv::imread(filepath1, 0);
       // cv::Mat templ1 = cv::imread(filepath2, 0);
        if (templ.data == NULL /*|| templ1.data == NULL*/)
            return checkerRe;
        //cv::resize(templ, templ, templ.size() / 4);
        //模板定，坐标定
        vector<cv::Point2f> pts1 = matchTemplateMaxLocs(roi, templ, 0.5, binNum);
        drawPointsOnImage(img_draw1, pts1);
        vector<cv::Point2f> ptsNew = pointsClusters(pts1, 30);
        cv::Mat indexMap1 = generatePointsIndexMap(ptsNew, false, 120 / binNum);
        indexMap = updateIndexMapKL(indexMap1, ptsNew, corners);
        ...
        for (int i = 0; i < indexMap.rows; i++)
        {
            for (int j = 0; j < indexMap.cols; j++)
            {
                ...
                if (in >= 0)
                {
                    circle(img_draw, corners[in], 5, Scalar(0, 0, 255), -1);
                }
            }
        }
...
vector<cv::Point2f> algorithm::MLCherkerboardDetect::matchTemplateMaxLocs(cv::Mat img, cv::Mat templ, double score,
    int bin)
{
    ...
    {
        double area = contourArea(contours[i], false);
        if (area > 1e2 / pow(bin, 2) & area < 1e5 / pow(bin, 2))
        {
            // cout << area << endl;
...
vector<cv::Rect> algorithm::MLCherkerboardDetect::getCRROI(vector<cv::Point2f> corners, cv::Mat indexMap,
...
bool algorithm::MLCherkerboardDetect::getNewROIRect(vector<Rect> roiVecP, vector<Rect> roiVecN, vector<Rect>& NewroiVecP,
...
int algorithm::MLCherkerboardDetect::findNegtiveNum(vector<int> list)
{
    int len = 0;
...
int algorithm::MLCherkerboardDetect::findMapNegNum(cv::Mat list)
{
    int len = 0;
...
bool algorithm::MLCherkerboardDetect::searchColIndex(cv::Mat indexMap, int& start, int& end)
{
    bool flag = true;
    int row = indexMap.rows;
    int col = indexMap.cols;
...
bool algorithm::MLCherkerboardDetect::searchRowIndex(cv::Mat indexMap, int& start, int& end)
{
    bool flag = true;
    int row = indexMap.rows;
    int col = indexMap.cols;
...
cv::Mat algorithm::MLCherkerboardDetect::updateIndexMap(cv::Mat indexMap, vector<cv::Point2f> cornersPoint,
    vector<cv::Point2f>& cornersPointNew)
{
    int row = indexMap.rows;
    int col = indexMap.cols;
...
cv::Mat algorithm::MLCherkerboardDetect::updateIndexMapKL(cv::Mat indexMap, vector<cv::Point2f> cornersPoint,
    vector<cv::Point2f>& cornersPointNew)
{
...
bool algorithm::MLCherkerboardDetect::getXYcoordinate(cv::Mat indexMap, vector<cv::Point2f> cornersPoint, cv::Mat& x,
    cv::Mat& y)
{
    bool flag = true;
    int row = indexMap.rows;
...
bool algorithm::MLCherkerboardDetect::getCenter(cv::Mat ip, cv::Mat& p, cv::Mat& px1, cv::Mat& px2, cv::Mat& px3,
    cv::Mat& px4)
{
    bool flag = true;
    int row = ip.rows;
...
cv::Mat algorithm::MLCherkerboardDetect::getArea(cv::Mat x1, cv::Mat x2, cv::Mat x3, cv::Mat x4, cv::Mat y1, cv::Mat y2,
    cv::Mat y3, cv::Mat y4)
{
...
bool algorithm::MLCherkerboardDetect::ReoderROIVec(vector<cv::Rect> roiVecP, vector<cv::Rect> roiVecN,
...
int algorithm::MLCherkerboardDetect::findNearstList(vector<cv::Rect> roiVecN, cv::Rect temp)
{
    int list = -1;
    vector<double> dis;
...
std::vector<double> algorithm::MLCherkerboardDetect::CalculateROIValue(cv::Mat img, vector<cv::Rect> roiVec)
{
    vector<double> crVec;
    if (img.data != NULL)
    {
...
std::vector<double> algorithm::MLCherkerboardDetect::CalculateCR(vector<double> posValue, vector<double> negValue)
{
    vector<double> crVec;
...
void algorithm::MLCherkerboardDetect::updateImgdraw(cv::Mat& imgdraw, vector<cv::Point2f> pts)
{
    for (int i = 0; i < pts.size(); i++)
    {
...
cv::Point2f algorithm::MLCherkerboardDetect::getPtsCenter(vector<cv::Point2f>pts)
{
    cv::Point2f c0;
...
void algorithm::MLCherkerboardDetect::SetCRAreaPercent(double percent)
{
    m_CRAreaPercent = percent;
}
void algorithm::MLCherkerboardDetect::SetChessboardSize(cv::Size size)
{
    m_boardsize = size;
}
