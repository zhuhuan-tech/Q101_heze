...
algorithm::MLRigidTrans3D::MLRigidTrans3D()
{
}
...
EyeboxACS algorithm::MLRigidTrans3D::getEyboxLocation(FiducialBase *f, vector<cv::Point3f> eyebox)
{
    EyeboxACS re;
    //if (f->manualFlag)
    //{
...
EyeboxACS algorithm::MLRigidTrans3D::getEyeboxLocationManual(FiducialManual *f, vector<cv::Point3f> eyebox)
{
    EyeboxACS re;
    vector<cv::Point3f> srcPoints, dstPoints;
    srcPoints = f->fiudcialWafer;
...
EyeboxACS algorithm::MLRigidTrans3D::getEyeboxLocationAuto(FiducialAuto *f, vector<cv::Point3f> eyebox)
{
    EyeboxACS re;
    vector<cv::Point3f> srcPoints, dstPoints;
    srcPoints = f->fiudcialWafer;
...
RigidTrans3D algorithm::MLRigidTrans3D::getRigidTrans3D(std::vector<cv::Point3f> srcPoints,
...
vector<cv::Point3f> algorithm::MLRigidTrans3D::getTranslationLoc(RigidTrans3D trans, vector<cv::Point3f> src)
{
...
cv::Point3f algorithm::MLRigidTrans3D::getFiducialACSLoc(cv::Point2f fiducialPixel, cv::Point3f fiducialACS)
{
    cv::Point3f pt;
...
cv::Point3f algorithm::MLRigidTrans3D::centroid(std::vector<cv::Point3f> pt)
{
    double srcSumX = 0.0f;
    double srcSumY = 0.0f;
    double srcSumZ = 0.0f;
...
Eigen::Vector3d algorithm::MLRigidTrans3D::rotationMatrixToEulerAngles(Eigen::Matrix3d &R)
{
