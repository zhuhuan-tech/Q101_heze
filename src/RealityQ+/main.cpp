#define RELATIVE_PLUGIN_PATH "plugins"
#define RELATIVE_LIBEXEC_PATH "."
#define RELATIVE_DATA_PATH "."
...
{
    if (qApp)
        QMessageBox::information(nullptr, QLatin1String(Core::Constants::APP_DISPLAY_NAME), toHtml(t));
    else
...
{
    if (qApp)
        QMessageBox::critical(nullptr, QLatin1String(Core::Constants::APP_DISPLAY_NAME), t);
    else
...
static void printVersion(const PluginSpec* coreplugin)
{
    ...
    str << '\n' << Core::Constants::APP_DISPLAY_NAME << ' ' << coreplugin->version() << "\n\n";
...
static inline int askMsgSendFailed()
{
    return QMessageBox::question(nullptr, QApplication::translate("Application", "Could not send message"),
        QCoreApplication::translate("Application",
            "Unable to send command line arguments "
            "to the already running instance. It does not appear to "
            "be responding. Do you want to start a new instance of "
            "%1?")
        .arg(Core::Constants::APP_DISPLAY_NAME),
        QMessageBox::Yes | QMessageBox::No | QMessageBox::Retry, QMessageBox::Retry);
}
...
static inline QStringList getPluginPaths()
{
    ...
    pluginPath += QLatin1Char('/') + QLatin1String(Core::Constants::IDE_SETTINGSVARIANT_STR) + QLatin1Char('/');
    pluginPath += Core::Constants::APP_ID;
    pluginPath += QLatin1String("/plugins/");
    // Qt Creator X.Y.Z can load plugins from X.Y.(Z-1) etc, so add current and previous
    // patch versions
    const QString minorVersion = QString::number(APP_VERSION_MAJOR) + '.' + QString::number(APP_VERSION_MINOR) + '.';
    const int minPatchVersion =
        qMin(APP_VERSION_RELEASE, QVersionNumber::fromString(Core::Constants::APP_VERSION_COMPAT).microVersion());
    for (int patchVersion = APP_VERSION_RELEASE; patchVersion >= minPatchVersion; --patchVersion)
...
static void setupInstallSettings(QString& installSettingspath)
{
    ...
    {
        displayError(QString("-installsettingspath \"%0\" needs to be the path where a %1/%2.ini exist.")
            .arg(installSettingspath, QLatin1String(Core::Constants::IDE_SETTINGSVARIANT_STR),
                QLatin1String(Core::Constants::APP_CASED_ID)));
...
{
    return new Utils::QtcSettings(QSettings::IniFormat, QSettings::UserScope,
        QLatin1String(Core::Constants::IDE_SETTINGSVARIANT_STR),
        QLatin1String(Core::Constants::APP_CASED_ID));
}
...
static void setHighDpiEnvironmentVariable()
{
    ...
    if (enableHighDpiScaling &&
        ...
        && !qEnvironmentVariableIsSet("QT_AUTO_SCREEN_SCALE_FACTOR") && !qEnvironmentVariableIsSet("QT_SCALE_FACTOR") &&
...
bool startCrashpad(const QString& libexecPath, bool crashReportingEnabled)
{
    ...
    annotations["app-version"] = Core::Constants::APP_VERSION_DISPLAY;
    ...
    bool success = client->StartHandler(handler, database, database, url, annotations, arguments,
        /* restartable */ true,
...
int main(int argc, char** argv)
{
    ...
    QString currPath = QString::fromStdString(GetExeDirectory().string());
    QString pluginsPath = currPath + "plugins";
    QString MILPath = pluginsPath + "/mil_DLL";
    QString path = qgetenv("Path") + ";" + pluginsPath + ";" + MILPath + ";" + currPath;
    bool res = qputenv("path", path.toStdString().c_str());
    if (res)
    {
        qDebug() << "add environment variable successfully!";
    }
    ...
    if (qEnvironmentVariableIsSet("DISABLE_NATIVE_MENUBAR") ||
    ...
    // Add qdebug log
    qInstallMessageHandler(CLog::myMessageOutput);
    SharedTools::QtSingleApplication::setAttribute(Qt::AA_ShareOpenGLContexts);

    int numberofArguments = static_cast<int>(options.appArguments.size());

    SharedTools::QtSingleApplication app((QLatin1String(Core::Constants::APP_DISPLAY_NAME)), numberofArguments,
        options.appArguments.data());
    QCoreApplication::setApplicationName(Core::Constants::APP_CASED_ID);
    ...
    QGuiApplication::setApplicationDisplayName(Core::Constants::APP_DISPLAY_NAME);
    ...
    Utils::QtcSettings* globalSettings = new Utils::QtcSettings(QSettings::IniFormat, QSettings::SystemScope,
        QLatin1String(Core::Constants::IDE_SETTINGSVARIANT_STR),
        QLatin1String(Core::Constants::APP_CASED_ID));
    loadFonts();

    QFont font;
    font.setFamily("Microsoft YaHei");  // 设置字体微软雅黑
    app.setFont(font);  // 应用到整个应用程序

    if (!qFuzzyCompare(qApp->devicePixelRatio(), 1.0) &&
        QApplication::style()->objectName().startsWith(QLatin1String("windows"), Qt::CaseInsensitive))
    {
    ...
    PluginManager::setPluginIID(QLatin1String("MLPlugins"));
    ...
    app.setDesktopFileName("org.mlo.desktop");
    ...
    const QString settingspath =
        options.preAppArguments.contains(QLatin1String(SETTINGS_OPTION)) ? QString() : options.settingsPath;
    const PluginManager::ProcessData processData = { restarter.executable(),
                                                    options.preAppArguments + PluginManager::argumentsForRestart(),
                                                    restarter.workingPath(), settingspath };
    ...
    if (foundAppOptions.contains(QLatin1String(HELP_OPTION1)) ||
        ...
        foundAppOptions.contains(QLatin1String(HELP_OPTION3)) || foundAppOptions.contains(QLatin1String(HELP_OPTION4)))
    {
        printHelp(QFileInfo(app.applicationFilePath()).baseName());
        return 0;
    }
    ...
    if (coreplugin->hasError())
    {
        if (!coreplugin->arguments().contains("Login cancel"))
        {
            displayError(msgCoreLoadFailure(coreplugin->errorString()));
        }
        return 1;
    ...
    QObject::connect(&app, &SharedTools::QtSingleApplication::messageReceived, &pluginManager,
